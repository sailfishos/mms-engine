...
static
void
test_delivery_ind_done_finish(
    Test* test)
{
    const TestDesc* desc = test->desc;
    MMS_DELIVERY_STATUS ds = mms_handler_test_delivery_status
        (test->handler, test->id);

    g_assert_cmpint(ds, == ,desc->status);
    ...
    MMSDispatcher* dispatcher)
{
    ...
    if (!mms_handler_test_receive_pending(test->handler, NULL)) {
        test_delivery_ind_done_finish(test);
    }
}
    ...
    gconstpointer data)
{
    ...
    test_dirs_init(&dirs, "test_delivery_ind");
    ...
    test.delegate.fn_done = test_delivery_ind_done;
...
int main(int argc, char* argv[])
{
    ...
    for (i = 0; i < G_N_ELEMENTS(delivery_tests); i++) {
